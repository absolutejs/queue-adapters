...
export const buildPostgresJobStore = <const Def extends JobDefinition>(
	...
	return {
		...
		enqueue: async (input) => {
			...
			// Atomic dedupe: the partial unique index guarantees one active job
			// per key even under concurrent enqueues. On conflict, return the
			// existing one.
		...
		listByKind: async (kind, options) => {
			...
			return rows.map((row) => toJob<Jobs>(row)) as Job<
				Jobs,
				typeof kind
			>[];
		},
